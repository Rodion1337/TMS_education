...
def upd_slug(sender, instance, **kwargs):
    if instance.slug == None:
        x = instance.name
        # text = (i for i in x if i.isalnum() or i == ' ')
        # print (text)
        # instance.slug = '_'.join(str(text).lower().split(' '))
        ...
        # translate = ''
        # for i in instance.slug:
        #     translate += dic.get(i.lower(), i.lower()).upper() if i.isupper() else dic.get(i, i)
        # instance.slug = translate
        instance.slug = '_'.join(x.lower().split(' '))