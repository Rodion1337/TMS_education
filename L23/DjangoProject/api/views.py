...
class GetGameInfoView(APIView):
    '''
    Данная в зависимости от передаваемых параметров осуществляет передачу необходимых данных в формате  json.<br>
    Поддерживаемые ключи:<br>
    sort - сортировка данных, по умолчанию данные выдаются без сортировки,
        поддерживается сортировка по цене (price) и названию (name)<br>
    year - сортировка игр вышедших в определенном году<br>
    random - при значение true передает одну случайную игру<br>
    пагинация - вывод раздробленных отсортированных данных по желаемому количеству частями:<br>
        page - номер страницы / части (по умолчания 1)<br>
        amount - количество записей в 1 части / странице (по умолчанию 5)<br>
    '''
...
class GetCommentInfoView(APIView):
    ...
    def get(self, request):
        if request.method == 'GET':
            request_result = request.GET
            game_id = request_result.get('game') if request_result.get('game') else 0
            queryset = Comments.objects.filter(game=int(game_id)) if Comments.objects.filter(game=int(game_id)) else Comments.objects.all()
            comment_serializer = CommentsSerializer(instance=queryset, many=True)
